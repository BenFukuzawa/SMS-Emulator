...
module Make (Bus : Word_addressable_intf.S) = struct
  ...
  let execute (t : t) (inst_info : Inst_info.t) : int =
    ...
    let write : type a. a Instruction.arg -> a -> unit =
      ...
      | SP -> t.sp <- y
      | SP_offset _ | Immediate16 _ | Immediate8 _ ->
        failwith @@ Printf.sprintf "Invalid arugment"
    in
    ...
    let next_pc =
      ...
      | ADD16 (x, y) ->
        ...
        set_flags
          ~h:
            Uint16.(
              (x' land of_int 0x0FFF) + (y' land of_int 0x0FFF)
              > of_int 0x0FFF)
          ~n:false
      ...
      | ADC (x, y) ->
        let c =
          if Registers.(read_flag t.registers Carry)
          then Uint8.one
          else Uint8.zero
        in
        let x', y' = read x, read y in
        ...
        set_flags
          ...
          ~h:
            Uint8.(
              (x' land of_int 0xF) + (y' land of_int 0xF) + c > of_int 0xF)
          ~n:false
      ...
      | SWAP x ->
        ...
        set_flags ~z:(n = Uint8.zero) ~h:false ~c:false ~n:false ();
        x <-- n;
        Next
      | DAA ->
        ...
        (match n_flag with
         | false ->
           if c_flag || !a > of_int 0x99
           then (
             a := !a + of_int 0x60;
             set_flags ~c:true ());
           if h_flag || !a land of_int 0x0F > of_int 0x09
           then a := !a + of_int 0x06
         | true ->
           if c_flag then a := !a - of_int 0x60;
           if h_flag then a := !a - of_int 0x06);
      ...
      | RR x ->
        ...
        set_flags
          ...
          ~h:false
          ~z:Uint8.(n = zero)
          ~c:Uint8.(x' land of_int 0x1 <> zero)
          ();
        Next
      | SLA x ->
        let x' = read x in
        ...
        set_flags
          ...
          ~h:false
          ~z:Uint8.(n = zero)
          ~c:Uint8.(x' land of_int 0x80 <> zero)
          ();
        Next
      | SRA x ->
        let x' = read x in
      ...
      | PUSH rr ->
        ...
        Bus.write_word
          t.bus
          ~addr:t.sp
          ~data:(Registers.read_rr t.registers rr);
        Next
      | POP rr ->
        Registers.write_rr t.registers rr (Bus.read_word t.bus t.sp);
        t.sp <- Uint16.(t.sp + of_int 2);
        Next
      | JP (c, x) -> if check_condition t c then Jump (read x) else Next
      | JR (c, x) ->
        if check_condition t c
        then (
    ...
    let handle_interrupt t : int =
      ...
      | Some type_ ->
        ...
        then 0
          ...
          5)
    in
    if t.enable_ime_before_next_instr
    then (
      t.ime <- true;
      t.enable_ime_before_next_instr <- false);
