...
module Make (Bus : Word_addressable_intf.S) = struct
  (* module RST_offset = struct let x00 = 0x00 |> Uint16.of_int let x08 =
     0x08 |> Uint16.of_int let x10 = 0x10 |> Uint16.of_int let x18 = 0x18 |>
     Uint16.of_int let x20 = 0x20 |> Uint16.of_int let x28 = 0x28 |>
     Uint16.of_int let x30 = 0x30 |> Uint16.of_int let x38 = 0x38 |>
     Uint16.of_int end *)
       ...
       | 0 ->
         (match y with
          ...
          | 4 | 5 | 6 | 7 ->
            ...
            ; inst = JR (Some (cc (y - 4)), Int8.of_byte (next_byte ()))
            }
          | _ -> assert false)
       | 1 ->
         (match q with
          ...
          | 1 ->
            (match p with
             ...
             | 2 ->
               { len = l3
               ; tcycles = { taken = 16; not_taken = 16 }
               ; inst = LD16 (RR Registers.HL, Direct16 (next_word ()))
               }
             | 3 ->
               { len = l3
               ; tcycles = { taken = 13; not_taken = 13 }
    ...
    | 1 ->
      (match z with
       | 6 ->
         (match y with
          | 6 ->
            { len = l1; tcycles = { taken = 4; not_taken = 4 }; inst = HALT }
          | _ ->
            { len = l1
            ; tcycles =
                (if y = 6 || z = 6
    ...
    | 3 ->
      (match z with
          ...
          | 1 ->
            (match p with
             ...
             | 3 ->
               { len = l1
               ; tcycles = { taken = 6; not_taken = 6 }
               ; inst = LD16 (RR Registers.SP, RR Registers.HL)
               }
             | _ -> assert false)
          | _ -> assert false)
       | 2 ->
         { len = l3
         ; tcycles = { taken = 10; not_taken = 10 }
         ; inst = JP (Some (cc y), Immediate16 (next_word ()))
         }
       | 3 ->
         (match y with
          | 0 ->
            { len = l3
            ; tcycles = { taken = 10; not_taken = 10 }
            ; inst = JP (None, Immediate16 (next_word ()))
            }
          | 1 -> assert false
          | 2 ->
            { len = l2
            ; tcycles = { taken = 11; not_taken = 11 }
            ; inst = OUT (Port_n (next_byte ()), Out_register Registers.A)
            }
          | 3 ->
            { len = l2
            ; tcycles = { taken = 11; not_taken = 11 }
            ; inst = IN (Some Registers.A, Port_n (next_byte ()))
            }
          | 4 ->
            { len = l1
            ; tcycles = { taken = 19; not_taken = 19 }
            ; inst = EX_SP_indirect Registers.HL
            }
          | 5 ->
            { len = l1
            ; tcycles = { taken = 4; not_taken = 4 }
          ...
          | 6 ->
            { len = l1; tcycles = { taken = 4; not_taken = 4 }; inst = DI }
          | 7 ->
            { len = l1; tcycles = { taken = 4; not_taken = 4 }; inst = EI }
          | _ -> assert false)
       | 4 ->
         { len = l3
         ; tcycles = { taken = 17; not_taken = 10 }
         ; inst = CALL (Some (cc y), next_word ())
         }
       | 5 ->
         (match q with
          | 0 ->
          ...
          | 1 ->
            (match p with
             | 0 ->
               { len = l3
               ; tcycles = { taken = 17; not_taken = 17 }
               ; inst = CALL (None, next_word ())
               }
             | 1 | 2 | 3 -> assert false
             | _ -> assert false)
          | _ -> assert false)
       | 6 ->
         { len = l2
         ; tcycles = { taken = 7; not_taken = 7 }
         ; inst = Lookup.alu.(y) (Immediate8 (next_byte ()))
         }
       | 7 ->
         { len = l1
         ; tcycles = { taken = 11; not_taken = 11 }
         ; inst = RST (Uint16.of_int (y * 8))
         }
       | _ -> assert false)
    | _ -> assert false
  ;;
  ...
  let decode_index_cb bus ~pc ~cb_offset index =
    (* let open Instruction_length in *)
    let displacement =
      Int8.of_byte (read_byte_at bus ~pc ~offset:(cb_offset + 1))
    in
    let opcode =
    ...
    let indexed_operand =
      match index with
      | Lookup.IX -> IX_indirect displacement
      | _ -> IY_indirect displacement
    ...
    | 0 ->
      ...
      ; inst = (Lookup.rot indexed_operand destination).(y)
      }
    | 1 ->
      { len
  ...
  let decode_index_opcode bus ~pc ~opcode_offset index opcode : Inst_info.t =
    ...
    let read_disp () = read_byte_at bus ~pc ~offset:(opcode_offset + 1) in
    let r idx =
    ...
    | 3, 1, 0, _ -> mk ~len:(len ~imm:0) ~t:10 (POP (rp2 p))
    ...
    | 3, 5, 0, _ -> mk ~len:(len ~imm:0) ~t:11 (PUSH (rp2 p))
    | _ -> assert false
  ;;
  ...
  let decode_cb bus ~pc : Inst_info.t =
    ...
    | 0 ->
      ...
      ; inst = (Lookup.rot operand None).(y)
      }
    | 1 ->
  ...
  let rec decode_index bus ~pc ~opcode_offset index =
    ...
    match opcode with
    | 0xDD ->
      decode_index bus ~pc ~opcode_offset:(opcode_offset + 1) Lookup.IX
    | 0xFD ->
      decode_index bus ~pc ~opcode_offset:(opcode_offset + 1) Lookup.IY
